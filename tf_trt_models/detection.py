...
from .graph_utils import expand_dim as f_expand_dim
...
def download_detection_model(model, output_dir='.'):
    ...
    print("Readownload_detection_model")
    global MODELS
...
def build_detection_graph(config, checkpoint,
    ...
    if not os.path.exists(os.path.join(output_dir, FROZEN_GRAPH_NAME)):
        print("Makes New Graph")
        ...
        print("Using Exist Exported Graph")
...
# for Non optimized traied model
def read_trained_model(TRAINED_MODEL, TRAINDED_DIR_PATH='./data/trained_model/',
    force_nms_cpu=True,
    replace_relu6=True,
    remove_assert=True
):
    print("Read trained %s Model" % (TRAINED_MODEL))
    # read frozen graph from file
    frozen_graph = tf.GraphDef()
    with open(os.path.join(TRAINDED_DIR_PATH, TRAINED_MODEL), 'rb') as f:
        frozen_graph.ParseFromString(f.read())
...
def read_optimized_model(model, frozen_dir='./dest/frozen/'):
    print("Read optimized %s Model" % (model))

    trt_graph = tf.GraphDef()
    with open(os.path.join(frozen_dir, model+'_trt.pb'), 'rb') as f:
